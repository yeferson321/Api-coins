...
const addCoinFavorites = (uuid: string, name: string): void => {
    if (localCoinStorage.value.includes(`&uuids[]=${uuid}&name=${name.toLowerCase()}`)) {
        ...
        localCoinStorage.value.push(`&uuids[]=${uuid}&name=${name.toLowerCase()}`);
    };
...
<template>
                    ...
                    <tr v-else class="border-b border-gray-700" v-for="(cryptos, index) in coins" :key="index">
                        ...
                        <th scope="row" class="px-1 py-2">
                            ...
                            <div class="flex items-center space-x-3 sm:space-x-4">
                                ...
                                <span>
                                    <button class="focus:outline-none" type="button" @click="addCoinFavorites(cryptos.uuid, cryptos.name)">
    ...
    <Pagination v-if="stats.total" @emitsOffset="offset = $event"></Pagination>
