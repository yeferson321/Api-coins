...
const { offset, halfItem } = toRefs(paginationStore);
...
const fetchMoreCoins = async (favoriteCoin: string[], offset: number) => {    
    ...
    try {
        const { coins: fetchedCoins }: DataInterface = await getSearchFavoritesCoins(favoriteCoin, offset, halfItem.value);
...
onMounted(() => {
    paginationStore.setOffset(halfItem.value);
